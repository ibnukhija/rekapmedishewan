    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <!-- Form Hapus -->
                                <form action="{{ route('pelayanan.destroy', $pelayanan->id_pelayanan) }}" method="POST" class="m-0 p-0">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50 dark:bg-gray-800/50">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
                Menampilkan {{ $pelayanans->firstItem() ?? 0 }} hingga {{ $pelayanans->lastItem() ?? 0 }} dari {{ $pelayanans->total() }} data
            </span>
            ...
            <div class="flex items-center gap-1">
                ...
                @else
                    <a href="{{ $pelayanans->previousPageUrl() }}" class="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 bg-white dark:bg-gray-800 transition-colors">Sebelumnya</a>
                @endif

                <!-- Tombol Halaman Saat Ini -->
                <button class="px-3 py-1 border border-brand-primary bg-brand-primary/10 text-brand-primary font-medium rounded-lg text-sm">{{ $pelayanans->currentPage() }}</button>

                <!-- Tombol Selanjutnya -->
                @if ($pelayanans->hasMorePages())
                    <a href="{{ $pelayanans->nextPageUrl() }}" class="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 bg-white dark:bg-gray-800 transition-colors">Selanjutnya</a>
                @else
...
<script>
    ...
    Swal.fire({
        ...
        text: "Data pelayanan yang dihapus tidak dapat dikembalikan!",
        icon: 'warning',
