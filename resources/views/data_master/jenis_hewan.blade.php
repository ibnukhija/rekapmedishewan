...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <!-- Search Form -->
        <form action="{{ route('jenis_hewan.index') }}" method="GET" class="relative w-full sm:w-96">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <i class="fa-solid fa-search text-gray-400"></i>
            </div>
            <input type="text" name="cari" value="{{ request('cari') }}" placeholder="Cari nama jenis hewan..."
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
                <thead class="bg-gray-50 dark:bg-gray-700/50 text-gray-700 dark:text-gray-300 font-semibold border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 whitespace-nowrap">Nama Jenis Hewan</th>
                        <th class="px-6 py-4 whitespace-nowrap text-center w-32">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">

                    @forelse ($jenisHewans as $index => $jenis)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <!-- Tombol Edit -->
                                <button onclick="openModal('edit', {{ $jenis }})" class="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 flex items-center justify-center transition-colors tooltip" title="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>

                                <!-- Form Hapus -->
                                <form action="{{ route('jenis_hewan.destroy', $jenis->id_jenis) }}" method="POST" class="m-0 p-0">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50 dark:bg-gray-800/50">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
                Menampilkan {{ $jenisHewans->firstItem() ?? 0 }} hingga {{ $jenisHewans->lastItem() ?? 0 }} dari {{ $jenisHewans->total() }} data
            </span>
            ...
            <div class="flex items-center gap-1">
                ...
                @else
                    <a href="{{ $jenisHewans->previousPageUrl() }}" class="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 bg-white dark:bg-gray-800 transition-colors">Sebelumnya</a>
                @endif

                <!-- Tombol Halaman Saat Ini -->
                <button class="px-3 py-1 border border-brand-primary bg-brand-primary/10 text-brand-primary font-medium rounded-lg text-sm">{{ $jenisHewans->currentPage() }}</button>

                <!-- Tombol Selanjutnya -->
                @if ($jenisHewans->hasMorePages())
                    <a href="{{ $jenisHewans->nextPageUrl() }}" class="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 bg-white dark:bg-gray-800 transition-colors">Selanjutnya</a>
                @else
...
<!-- Modal Form (Tambah/Edit Jenis Hewan) -->
<div id="jenisModal" class="modal-hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="modal-content relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-100 dark:border-gray-700">
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/50">
            <h3 id="modalTitle" class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <i class="fa-solid fa-paw text-brand-primary"></i>
                <span id="modalTitleText">Tambah Jenis Hewan</span>
            </h3>
        ...
        <form id="jenisForm" action="{{ route('jenis_hewan.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
            <!-- Method field untuk update akan diinjeksi via JS saat mode Edit -->
            <div id="method-container"></div>

            <div class="space-y-1.5">
                <label for="nama_jenis" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Jenis Hewan <span class="text-red-500">*</span></label>
                <input type="text" id="nama_jenis" name="nama_jenis" required placeholder="Contoh: Kucing, Anjing, Kelinci"
...
<script>
    ...
    const storeUrl = "{{ route('jenis_hewan.store') }}";
    const updateUrlBase = "{{ url('jenis-hewan') }}"; // Base URL untuk update (/jenis-hewan/{id})
    ...
    function openModal(mode, data = null) {
        ...
        if (mode === 'edit' && data) {
            modalTitleText.textContent = 'Edit Jenis Hewan';
            // Set action form ke URL Update
            form.action = updateUrlBase + '/' + data.id_jenis;
            ...
            modalTitleText.textContent = 'Tambah Jenis Hewan';
    ...
    Swal.fire({
        ...
        text: "Data jenis hewan yang dihapus tidak dapat dikembalikan!",
        icon: 'warning',
